# test Exp-CRBM on Fashion-MNIST
...
model_crbm=capsule_rbm.capsule_rbm(M=M,K=K, J=J, visible_type=visible_type, visible_type_fixed_param=visible_type_fixed_param, hidden_type=hidden_type, hidden_type_fixed_param=hidden_type_fixed_param, tie_W_for_pretraining_DBM_bottom=False, tie_W_for_pretraining_DBM_top=False, if_fix_vis_bias=False, a=None, xz_interaction=xz_interaction, rec_error_max=0.5, rng=rng)
...
shutil.copy(workdir+"main_test_ExpCRBM_FASHIONMNIST.py", dir_save)
...
# sort capsules
#num_sample_per_cl=1000
#train_set_x1000,train_set_y1000,_,_=cl.truncate_sample_size(train_set_x.transpose(),train_set_y,max_size_given=num_sample_per_cl)
#train_set_x1000=train_set_x1000.transpose()
#ZM,ZM_sorted,ind=model_crbm.infer_z_given_x(train_set_x_sub=train_set_x1000, NMF=100, num_col=50, plotimg=False, dir_save=dir_save, prefix=prefix+"_1000")
#model_crbm.reorder_capsules(ind)
...
#    _,_=model_crbm.per_capsule_pattern_given_x(train_set_x_sub=train_set_x100_split[c], NMF=1000, row=28, col=28, figwidth=12, figheight=6, dir_save=dir_save, prefix=prefix+"_class_"+str(c)+"_per_capsule")


# given x, infer z and h, then generate image for each active capsule
train_set_x100_split=numpy.hsplit(train_set_x100, num_cl )
for c in range(num_cl):
    _,_=model_crbm.per_capsule_pattern_given_x(train_set_x_sub=train_set_x100_split[c], NMF=1000, threshold_z=0, row=28, col=28, figwidth=12, figheight=6, dir_save=dir_save, prefix=prefix+"_class_"+str(c)+"_per_capsule_thresholdz_"+str(0))

# a different threshold
for c in range(num_cl):
    _,_=model_crbm.per_capsule_pattern_given_x(train_set_x_sub=train_set_x100_split[c], NMF=1000, threshold_z=0.1, row=28, col=28, figwidth=12, figheight=6, dir_save=dir_save, prefix=prefix+"_class_"+str(c)+"_per_capsule_thresholdz_"+str(0.1))
...
for k in range(1,K):
    ...
    Zk[k,:]=1
    Z=numpy.hstack((Z,Zk))
model_crbm.generate_samples_given_z(Z0=Z, NS=K*num_samples_per_capsule, sampling_time=4, reinit=False, pcdk=1000, rand_init=True, num_col=10, figwidth=10, figheight=2, dir_save=dir_save, prefix=prefix+"_only_one_capsule_active_")

## sampling using Gibbs sampling
#_,_,ind=model_crbm.generate_samples(NS=100, sampling_time=4, reinit=False, pcdk=1000, rand_init=True, dir_save=dir_save, prefix=prefix)
#
## get z code for 100 training samples
#num_sample_per_cl=10
#train_set_x100,train_set_y100,_,_=cl.truncate_sample_size(train_set_x.transpose(),train_set_y,max_size_given=num_sample_per_cl)
...
#ZM,ZM_sorted=model_crbm.generate_samples_given_x(train_set_x_sub=train_set_x100, NMF=1000, dir_save=dir_save, prefix=prefix)
#
## given z code to generate samples
#num_cl=10
#model_crbm.generate_samples_given_z(Z0=ZM, NS=num_sample_per_cl*num_cl, sampling_time=4, reinit=False, pcdk=1000, rand_init=True, dir_save=dir_save, prefix=prefix+"_given_ZM")
#
## generate 100 samples per class
#for s in range(num_cl):
#    Z0=numpy.tile(ZM[:,s*num_sample_per_cl:(s+1)*num_sample_per_cl], num_sample_per_cl)
...
#    Zk[k,:]=1
#    Z=numpy.hstack((Z,Zk))
#model_crbm.generate_samples_given_z(Z0=Z, NS=K*10, sampling_time=4, reinit=False, pcdk=1000, rand_init=True, num_col=10, figwidth=24, figheight=24*K/num_samples_per_capsule, dir_save=dir_save, prefix=prefix+"_only_one_capsule_active")
