...
class capsule_hm:
    def __init__(self, features=None, M=None, K=None, J=16, visible_type="Bernoulli", visible_type_fixed_param=1, hidden_type="Bernoulli", hidden_type_fixed_param=0, if_fix_vis_bias=False, a=None, fix_a_log_ind=None, tol_poisson_max=8, rec_error_max=0.5, xz_interaction=True, rng=numpy.random.RandomState(100)):
        """
...
#        for l in range(self.L):
#            if l==0 and self.L>1: # first
#                crbm=capsule_rbm3.capsule_rbm3(features=self.features, M=self.M, K=self.K[0], J=self.J[0], visible_type=self.visible_type, visible_type_fixed_param=self.visible_type_fixed_param, hidden_type=self.hidden_type[0], hidden_type_fixed_param=self.hidden_type_fixed_param[0], tie_W_for_pretraining_DBM_bottom=True, tie_W_for_pretraining_DBM_top=False, if_fix_vis_bias=if_fix_vis_bias, a=a, fix_a_log_ind=fix_a_log_ind, xz_interaction=self.xz_interaction, tol_poisson_max=tol_poisson_max, rng=self.rng)
#                a,b,c,W=crbm.get_param()
#                self.a=a
#                self.b[l]=b
#                self.c[l]=c
#                self.W[l]=W
...
#                self.Wr[l]=self.transpose_Wl(self.W[l], l=0, K1=None, K2=None, xz_interaction=self.xz_interaction)
#                self.crbms[l]=crbm
#            elif l==self.L-1 and self.L>1: # last
#                crbm=capsule_rbm23.capsule_rbm23(features=None, M=self.K[l-1], K=self.K[l], I=self.J[l-1], J=self.J[l], visible_type=self.hidden_type[l-1], visible_type_fixed_param=self.hidden_type_fixed_param[l-1], hidden_type=self.hidden_type[l], hidden_type_fixed_param=self.hidden_type_fixed_param[l], tie_W_for_pretraining_DBM_bottom=False, tie_W_for_pretraining_DBM_top=True, if_fix_vis_bias=True, a=self.b[l-1], d=self.c[l-1], fix_a_log_ind=None, xz_interaction=self.xz_interaction, tol_poisson_max=tol_poisson_max, rng=self.rng) # if_fix_vis_bias=True
#                _,b,c,_,W=crbm.get_param()
#                self.b[l]=b
#                self.c[l]=c
#                self.W[l]=W
#                self.br[l]=copy.deepcopy(self.b[l])
#                self.cr[l]=copy.deepcopy(self.c[l])
#                self.Wr[l]=self.transpose_Wl(self.W[l], l=self.L-1, K1=self.K[l-1], K2=self.K[l], xz_interaction=self.xz_interaction)
#                self.crbms[l]=crbm
#            elif l==0 and self.L==1: # shallow
#                crbm=capsule_rbm3.capsule_rbm3(features=self.features, M=self.M, K=self.K[0], J=self.J[0], visible_type=self.visible_type, visible_type_fixed_param=self.visible_type_fixed_param, hidden_type=self.hidden_type[0], hidden_type_fixed_param=self.hidden_type_fixed_param[0], tie_W_for_pretraining_DBM_bottom=False, tie_W_for_pretraining_DBM_top=False, if_fix_vis_bias=if_fix_vis_bias, a=a, fix_a_log_ind=fix_a_log_ind, xz_interaction=self.xz_interaction, tol_poisson_max=tol_poisson_max, rng=self.rng)
#                a,b,c,W=crbm.get_param()
#                self.a=a
#                self.b[l]=b
#                self.c[l]=c
...
#                self.crbms[l]=crbm
#            else: # middle
#                crbm=capsule_rbm23.capsule_rbm23(features=None, M=self.K[l-1], K=self.K[l], I=self.J[l-1], J=self.J[l], visible_type=self.hidden_type[l-1], visible_type_fixed_param=self.hidden_type_fixed_param[l-1], hidden_type=self.hidden_type[l], hidden_type_fixed_param=self.hidden_type_fixed_param[l], tie_W_for_pretraining_DBM_bottom=False, tie_W_for_pretraining_DBM_top=False, if_fix_vis_bias=True, a=self.b[l-1], d=self.c[l-1], fix_a_log_ind=None, xz_interaction=self.xz_interaction, tol_poisson_max=tol_poisson_max, rng=self.rng) #if_fix_vis_bias=True
#                _,b,c,_,W=crbm.get_param()
#                self.b[l]=b
#                self.c[l]=c
#                self.W[l]=W
#                self.br[l]=copy.deepcopy(self.b[l])
#                self.cr[l]=copy.deepcopy(self.c[l])
#                self.Wr[l]=self.transpose_Wl(self.W[l], l=l, K1=self.K[l-1], K2=self.K[l], xz_interaction=self.xz_interaction)
#                self.crbms[l]=crbm

        for l in range(self.L):
            if l==0 and self.L>1: # first
                crbm=capsule_rbm.capsule_rbm(features=self.features, M=self.M, K=self.K[0], J=self.J[0], visible_type=self.visible_type, visible_type_fixed_param=self.visible_type_fixed_param, hidden_type=self.hidden_type[0], hidden_type_fixed_param=self.hidden_type_fixed_param[0], tie_W_for_pretraining_DBM_bottom=False, tie_W_for_pretraining_DBM_top=False, if_fix_vis_bias=if_fix_vis_bias, a=a, fix_a_log_ind=fix_a_log_ind, xz_interaction=self.xz_interaction, rec_error_max=self.rec_error_max, tol_poisson_max=tol_poisson_max, rng=self.rng)
                a,b,c,W=crbm.get_param()
                self.a=a
                self.b[l]=b
                self.c[l]=c
                ...
                crbm=capsule_rbm.capsule_rbm(features=None, M=self.K[l-1]*self.J[l-1][0], K=self.K[l], J=self.J[l], visible_type=self.hidden_type[l-1], visible_type_fixed_param=self.hidden_type_fixed_param[l-1], hidden_type=self.hidden_type[l], hidden_type_fixed_param=self.hidden_type_fixed_param[l], tie_W_for_pretraining_DBM_bottom=False, tie_W_for_pretraining_DBM_top=False, if_fix_vis_bias=True, a=self.merge_bl(self.b[l-1], self.hidden_type[l-1]), fix_a_log_ind=None, xz_interaction=self.xz_interaction, rec_error_max=self.rec_error_max, tol_poisson_max=tol_poisson_max, rng=self.rng) # if_fix_vis_bias=True
                _,b,c,W=crbm.get_param()
                self.b[l]=b
                self.c[l]=c
                ...
                #How should I initialize and handle Omega for Z-Z interaction?
...
#                self.crbms[l].train(X=crbm_X, V=crbm_V, X_validate=None, V_validate=None, batch_size=batch_size, NMF=NMF, increase_NMF_at=increase_NMF_at, increased_NMF=increased_NMF, pcdk=pcdk, NS=NS, maxiter=maxiter, learn_rate_a=learn_rate_b[l-1], learn_rate_b=learn_rate_b[l], learn_rate_c=learn_rate_c[l], learn_rate_d=learn_rate_c[l-1], learn_rate_W=learn_rate_W[l], change_rate=change_rate, adjust_change_rate_at=adjust_change_rate_at, adjust_coef=adjust_coef, reg_lambda_a=0, reg_alpha_a=1, reg_lambda_b=0, reg_alpha_b=1, reg_lambda_W=0, reg_alpha_W=1, change_every_many_iters=change_every_many_iters, init_chain_time=init_chain_time, train_subset_size_for_compute_error=train_subset_size_for_compute_error, valid_subset_size_for_compute_error=valid_subset_size_for_compute_error, track_reconstruct_error=track_reconstruct_error, track_free_energy=track_free_energy, reinit_a_use_data_stat=False, if_plot_error_free_energy=if_plot_error_free_energy, dir_save=dir_save, prefix=prefix+"_"+str(l), figwidth=figwidth, figheight=figwidth)
#                _,b,c,_,W=self.crbms[l].get_param()
#                self.b[l]=b
#                self.c[l]=c
#                self.W[l]=W
#                self.br[l]=copy.deepcopy(self.b[l])
#                self.cr[l]=copy.deepcopy(self.c[l])
#                self.Wr[l]=self.transpose_Wl(self.W[l], l=l, K1=self.K[l-1], K2=self.K[l], xz_interaction=self.xz_interaction)
#                Xl,Vl,Hl,Zl=self.crbms[l].mean_field_approximate_inference(crbm_X, crbm_V, NMF=NMF)
#                crbm_X=Hl
#                crbm_V=Zl
#                self.H_pretrain[l]=Hl
#                self.Z_pretrain[l]=Zl
                self.crbms[l].fix_vis_bias( a=self.merge_bl(self.b[l-1],self.hidden_type[l-1]) )
...
#        return Wls
    ...
    def split_Wl(self, Wl, I, l):
        ...
        """
        K=len(Wl)
        K=K-1 # number of capsules at the upper layer
        ...
        for k in range(K):
            for k1 in range(K1):
                Wls[k1][k] = Wl[k][k1*I:(k1+1)*I,:]
        if self.xz_interaction:
            WlK=numpy.zeros((K1,K))# z-z interaction
            for k1 in range(K1):
                #cap_mean = numpy.mean(self.b[l-1][k1][0]/(-2*self.b[l-1][k1][1] + 1e-10))
                #WlK[k1,:]=numpy.sum(Wl[K][k1*I:(k1+1)*I,:], axis=0 ) / cap_mean
                WlK[k1,:]=numpy.mean(Wl[K][k1*I:(k1+1)*I,:], axis=0 )/I 
            Wls.append(WlK)
        else:
            Wls.append(None)
        #Wls.append(Wl[K]) # the pretraining does not learning  x-z and z-z interactions. Thus, keep the initialized values to the fine-tuning.
    ...
    def train(self, X=None, X_validate=None, batch_size=10, NMF=20, increase_NMF_at=None, increased_NMF=[20], maxiter=100, learn_rate_a=0.01, learn_rate_b=0.01, learn_rate_c=0.01, learn_rate_W=0.01, change_rate=0.8, adjust_change_rate_at=None, adjust_coef=1.02, change_every_many_iters=10, init_chain_time=100, train_subset_size_for_compute_error=100, valid_subset_size_for_compute_error=100, track_reconstruct_error=True, track_free_energy=True, if_plot_error_free_energy=False, dir_save="./", prefix="CHM", figwidth=5, figheight=5):
        """
        Wake-sleep algorithm to train Capsule-HM.
        Different layers can have different learning rates.
        """
        start_time=time.clock()
        print("Start training CHM...")
        ...
        for i in range(self.maxiter):
            ...
            XbatchMg,HbatchMg,ZbatchMg,Hbatchr,Zbatchr,a_hat_gen,b_hat_gen,c_hat_gen=self.sample_xhz_wake(Xbatch, NMF=NMF, value_or_meanfield="meanfield", compute_HMg=True)   
            self.compute_gradient_wake(Xbatch, Hbatchr, Zbatchr, XbatchMg, HbatchMg, ZbatchMg)
            ...
            Xfantacy,XfantacyM,Hfantacy,Zfantacy,HfantacyMr,ZfantacyMr=self.sample_xhz_sleep(NS=self.batch_size, Hg=copy.deepcopy(Hbatchr), Zg=copy.deepcopy(Zbatchr), warm_start=False, NMF=NMF, value_or_meanfield="meanfield", compute_HMr=True)
            
#            if i>=12000 and (i//100)%2==0: # every 100 iterations, try original sleep method
#                Xfantacy,XfantacyM,Hfantacy,Zfantacy,HfantacyMr,ZfantacyMr=self.sample_xhz_sleep(NS=self.batch_size, Hg=None, Zg=None, warm_start=False, NMF=NMF, value_or_meanfield="meanfield", compute_HMr=True)
#            else:
#                Xfantacy,XfantacyM,Hfantacy,Zfantacy,HfantacyMr,ZfantacyMr=self.sample_xhz_sleep(NS=self.batch_size, Hg=copy.deepcopy(Hbatchr), Zg=copy.deepcopy(Zbatchr), warm_start=False, NMF=NMF, value_or_meanfield="meanfield", compute_HMr=True) # I want to try this
            
            self.compute_gradient_sleep(Xfantacy, Hfantacy, Zfantacy,  HfantacyMr, ZfantacyMr) # try to replace Xfantacy by XfantacyM
            ...
            if track_reconstruct_error:
                rec_error_train,_,_,_,_,_=self.compute_reconstruction_error(X0=Xbatch, X0RM=XbatchMg )
            ...
            if self.X_validate is not None:
                if valid_subset_size_for_compute_error is not None:
                    ...
                    if track_reconstruct_error:
                        rec_error_valid,HMr_valid,ZMr_valid,a_hat_gen_valid,b_hat_gen_valid,c_hat_gen_valid=self.compute_reconstruction_error(X0=X_validate_subset, X0RM=None)
                    ...
                    if track_reconstruct_error:                  
                        rec_error_valid,HMr_valid,ZMr_valid,a_hat_gen_valid,b_hat_gen_valid,c_hat_gen_valid=self.compute_reconstruction_error(X0=self.X_validate, X0RM=None)
                        self.rec_errors_valid.append(rec_error_valid)
                    if track_free_energy:
...
#                    print("{0}-th iteration, learn_rate_W: {1}".format(i, self.learn_rate_W))
        ...
        print("The (fine-tuning) training of CHM is finished!")
        end_time = time.clock()
    ...
    def sample_xhz_wake(self, X, NMF=20, value_or_meanfield="meanfield", compute_HMg=True):
        """
        ...
        for l in range(self.L):
            if l==0:
                ...
                for n in range(NMF):
                    # reset c_hat
                    if self.xz_interaction:
                        c_hat[l] = self.cr[l] + self.Wr[l][self.K[l]] @ X
                    else:
                        c_hat[l] = numpy.tile(self.cr[l], NS)
                    for k in range(self.K[l]):
                        if n==0: # # initialize H[0]
                            H[l][k],_ = self.sample_h_given_b_hat(b_hat=self.repeat_bias(b_hat[l][k],NS,self.hidden_type[l]), hidden_type=self.hidden_type[l], hidden_type_fixed_param=self.hidden_type_fixed_param[l], hidden_value_or_meanfield=value_or_meanfield)
                        c_hat[l][k,:] = c_hat[l][k,:] + numpy.diag( H[l][k].T @ self.Wr[l][k] @ X )
                    # sample Z
                    Z[l],_=self.sample_h_given_b_hat(b_hat=c_hat[l], hidden_type="Bernoulli", hidden_type_fixed_param=0, hidden_value_or_meanfield=value_or_meanfield)
                    # b_hat
                    b_hat[l]=copy.deepcopy(self.br[l])
                    for k in range(self.K[l]):
                        if self.hidden_type[l]=="Bernoulli":
                            b_hat[l][k]= b_hat[l][k] + Z[l][k,:] * (self.Wr[l][k] @ X)
                        elif self.hidden_type[l]=="Gaussian_FixPrecision1":
                            pass
                        elif self.hidden_type[l]=="Gaussian":
                            b_hat[l][k][0] = b_hat[l][k][0] + Z[l][k,:] * (self.Wr[l][k] @ X)
                        # sample H
                        H[l][k],_ = self.sample_h_given_b_hat(b_hat=b_hat[l][k], hidden_type=self.hidden_type[l], hidden_type_fixed_param=self.hidden_type_fixed_param[l], hidden_value_or_meanfield=value_or_meanfield)
                ...
                for n in range(NMF):
                    # reset c_hat
                    if self.xz_interaction:
                        c_hat[l] = self.cr[l] +  self.Wr[l][self.K[l-1]] @ Z[l-1]
                    else:
                        c_hat[l] = numpy.tile(self.cr[l], NS)
                    for k in range(self.K[l]):
                        if n==0: # initialize H[l]
                            H[l][k],_ = self.sample_h_given_b_hat(b_hat=self.repeat_bias(b_hat[l][k],NS,self.hidden_type[l]), hidden_type=self.hidden_type[l], hidden_type_fixed_param=self.hidden_type_fixed_param[l], hidden_value_or_meanfield=value_or_meanfield)
                        for k1 in range(self.K[l-1]):
                            c_hat[l][k,:] = c_hat[l][k,:] + Z[l-1][k1,:]*numpy.diag( H[l][k].T @ self.Wr[l][k1][k] @ H[l-1][k1])
                    # sample Z
                    Z[l],_=self.sample_h_given_b_hat(b_hat=c_hat[l], hidden_type="Bernoulli", hidden_type_fixed_param=0, hidden_value_or_meanfield=value_or_meanfield)
                    ...
                    for k in range(self.K[l]):
                        if self.hidden_type[l]=="Bernoulli":
                            for k1 in range(self.K[l-1]):
                                b_hat[l][k]=b_hat[l][k] + Z[l][k,:]*Z[l-1][k1,:] * (self.Wr[l][k1][k] @ H[l-1][k1])
                        elif self.hidden_type[l]=="Gaussian_FixPrecision1":
                            pass
                        elif self.hidden_type[l]=="Gaussian":
                            for k1 in range(self.K[l-1]):
                                b_hat[l][k][0] = b_hat[l][k][0] + Z[l][k,:]*Z[l-1][k1,:] * (self.Wr[l][k1][k] @ H[l-1][k1])
                        # sample H
                        H[l][k],_ = self.sample_h_given_b_hat(b_hat=b_hat[l][k], hidden_type=self.hidden_type[l], hidden_type_fixed_param=self.hidden_type_fixed_param[l], hidden_value_or_meanfield=value_or_meanfield)
        ...
        b_hat_gen=[None]*self.L #copy.deepcopy(self.b)
        c_hat_gen=[None]*self.L #copy.deepcopy(self.c)
        ...
        if compute_HMg:
            for l in range(self.L-1,-1,-1):                       
                if l==self.L-1: # last hidden layer:
                    ...
                    # sample ZMg
                    ZMg[l],_=self.sample_h_given_b_hat(b_hat=c_hat_gen[l], hidden_type="Bernoulli", hidden_type_fixed_param=0, hidden_value_or_meanfield="meanfield")
                    # b_hat_gen
                    b_hat_gen[l]=self.repeat_b(self.b[l], NS, self.hidden_type[l])
                    # generate HMg
                    for k in range(self.K[l]):
                        HMg[l][k],_ = self.sample_h_given_b_hat(b_hat=b_hat_gen[l][k], hidden_type=self.hidden_type[l], hidden_type_fixed_param=self.hidden_type_fixed_param[l], hidden_value_or_meanfield="meanfield")
                    ...
                    for n in range(NMF): # mean-field
                        ...
                        for k in range(self.K[l]):
                            if n==0:
                                HMg[l][k],_ = self.sample_h_given_b_hat(b_hat=self.repeat_bias(self.b[l][k],NS,self.hidden_type[l]), hidden_type=self.hidden_type[l], hidden_type_fixed_param=self.hidden_type_fixed_param[l], hidden_value_or_meanfield="meanfield") # initialize HMg
                            for k2 in range(self.K[l+1]):
                                c_hat_gen[l][k,:]= c_hat_gen[l][k,:] + Z[l+1][k2,:]*numpy.diag( HMg[l][k].T @ self.W[l+1][k][k2] @ H[l+1][k2] )
                        # sample ZMg
                        ZMg[l],_=self.sample_h_given_b_hat(b_hat=c_hat_gen[l], hidden_type="Bernoulli", hidden_type_fixed_param=0, hidden_value_or_meanfield="meanfield")
                        # b_hat_gen
                        ...
                        for k in range(self.K[l]):
                            if self.hidden_type[l]=="Bernoulli":
                                for k2 in range(self.K[l+1]):
                                    b_hat_gen[l][k] = b_hat_gen[l][k] + ZMg[l][k,:]*Z[l+1][k2,:]*(self.W[l+1][k][k2] @ H[l+1][k2])
                            elif self.hidden_type[l]=="Gaussian_FixPrecision1":
                                pass
                            elif self.hidden_type[l]=="Gaussian":
                                for k2 in range(self.K[l+1]):
                                    b_hat_gen[l][k][0]= b_hat_gen[l][k][0] + ZMg[l][k,:]*Z[l+1][k2,:]*(self.W[l+1][k][k2] @ H[l+1][k2])
                            # generate HMg
                            HMg[l][k],_ = self.sample_h_given_b_hat(b_hat=b_hat_gen[l][k], hidden_type=self.hidden_type[l], hidden_type_fixed_param=self.hidden_type_fixed_param[l], hidden_value_or_meanfield="meanfield")
        
        
        return XMg,HMg,ZMg,H,Z,a_hat_gen,b_hat_gen,c_hat_gen
    ...
    def compute_gradient_wake(self, Xbatch, Hbatchr, Zbatchr,  XbatchMg, HbatchMg, ZbatchMg):
        """
        ...
        for l in range(self.L):
            if self.hidden_type[l] in ["Bernoulli", "Poisson", "Binomial", "Multinomial", "Gaussian_FixPrecision2"]:
                ...
                for k in range(self.K[l]):
                    grad_b_lk1=-numpy.mean(Hbatchr[l][k] - HbatchMg[l][k], axis=1)
                    grad_b_lk1.shape=(self.J[l][k],1)
                    HbatchMg_lk2=HbatchMg[l][k]**2 - 1/(2*self.b[l][k][1])
                    grad_b_lk2=-numpy.mean(Hbatchr[l][k]**2 - HbatchMg_lk2, axis=1) 
                    grad_b_lk2.shape=(self.J[l][k],1)
                    self.grad_b[l][k]=[grad_b_lk1,grad_b_lk2]
        ...
        for l in range(self.L):
            if l==0:
                ...
                for k1 in range(self.K[l-1]):
                    for k in range(self.K[l]):
                        grad_Wlk1k = - ( ((Zbatchr[l-1][k1,:]*Hbatchr[l-1][k1]) - (ZbatchMg[l-1][k1,:]*HbatchMg[l-1][k1])) @ (Zbatchr[l][k,:]*Hbatchr[l][k]).T )/self.batch_size
            ...
            if l==0:
                for k in range(self.K[l]):
                    if self.visible_type=="Multinoulli":
                        ...
                        self.W[l][k][0] = self.W[l][k][0] - self.learn_rate_W[l][0] * self.grad_W[l][k][0]
                        self.W[l][k][1] = self.W[l][k][1] - self.learn_rate_W[l][1] * self.grad_W[l][k][1]
                    else:
                ...
                for k1 in range(self.K[l-1]):
                    for k in range(self.K[l]):
                        if self.hidden_type[l]=="Gaussian2":
                            self.W[l][k1][k][0] = self.W[l][k1][k][0] - self.learn_rate_W[l][0] * self.grad_W[l][k1][k][0]
                            self.W[l][k1][k][1] = self.W[l][k1][k][1] - self.learn_rate_W[l][1] * self.grad_W[l][k1][k][1]
                        else:
    ...
    def sample_xhz_sleep(self, NS=100, Hg=None, Zg=None, warm_start=False, NMF=20, value_or_meanfield="meanfield", compute_HMr=True):
        """
        ...
        warm_start: logical, if Ture, use Hr/HMr and Zr/ZMr to initialize Hg and Zg. This requires Hg and Zg are not None as inputs.
        """
        if Hg is None:
            #Hg=[None]*self.L
            #for l in range(self.L):
        ...
        for l in range(last,-1,-1):
            if l==self.L-1:
                # generate Zg
                c_hat_gen[l] = numpy.tile(self.c[l], NS)
                Zg[l],_=self.sample_h_given_b_hat(b_hat=c_hat_gen[l], hidden_type="Bernoulli", hidden_type_fixed_param=0, hidden_value_or_meanfield="value")
                # generate Hg
                b_hat_gen[l]=self.repeat_b(self.b[l], NS, self.hidden_type[l])
                for k in range(self.K[l]):
                    Hg[l][k],_ = self.sample_h_given_b_hat(b_hat=b_hat_gen[l][k], hidden_type=self.hidden_type[l], hidden_type_fixed_param=self.hidden_type_fixed_param[l], hidden_value_or_meanfield="value")
                ...
                for n in range(NMF):
                    ...
                    for k in range(self.K[l]):
                        if n==0 and not warm_start:
                            Hg[l][k],_ = self.sample_h_given_b_hat(b_hat=self.repeat_bias(self.b[l][k],NS,self.hidden_type[l]), hidden_type=self.hidden_type[l], hidden_type_fixed_param=self.hidden_type_fixed_param[l], hidden_value_or_meanfield=value_or_meanfield) # initialize
                        for k2 in range(self.K[l+1]):
                            c_hat_gen[l][k,:]= c_hat_gen[l][k,:] + Zg[l+1][k2,:]*numpy.diag( Hg[l][k].T @ self.W[l+1][k][k2] @ Hg[l+1][k2] )
                    # generate Zg
                    Zg[l],_=self.sample_h_given_b_hat(b_hat=c_hat_gen[l], hidden_type="Bernoulli", hidden_type_fixed_param=0, hidden_value_or_meanfield=value_or_meanfield)
                    ...
                    for k in range(self.K[l]):
                        if self.hidden_type[l]=="Bernoulli":
                            for k2 in range(self.K[l+1]):
                                b_hat_gen[l][k] = b_hat_gen[l][k] + Zg[l][k,:]*Zg[l+1][k2,:]*(self.W[l+1][k][k2] @ Hg[l+1][k2])
                        elif self.hidden_type[l]=="Gaussian":
                            for k2 in range(self.K[l+1]):
                                b_hat_gen[l][k][0]= b_hat_gen[l][k][0] + Zg[l][k,:]*Zg[l+1][k2,:]*(self.W[l+1][k][k2] @ Hg[l+1][k2])
                        # generate Hg
                        Hg[l][k],_ = self.sample_h_given_b_hat(b_hat=b_hat_gen[l][k], hidden_type=self.hidden_type[l], hidden_type_fixed_param=self.hidden_type_fixed_param[l], hidden_value_or_meanfield=value_or_meanfield)
            ...
            for l in range(self.L):
                if l==0:
                    ...
                    for n in range(NMF):
                        # reset c_hat
                        if self.xz_interaction:
                            c_hat[l] = self.cr[l] + self.Wr[l][self.K[l]] @ Xg
                        else:
                            c_hat[l] = numpy.tile(self.cr[l], NS)
                        for k in range(self.K[l]):
                            if n==0 and not warm_start: # # initialize HMr[l]
                                HMr[l][k],_ = self.sample_h_given_b_hat(b_hat=self.repeat_bias(b_hat[l][k],NS,self.hidden_type[l]), hidden_type=self.hidden_type[l], hidden_type_fixed_param=self.hidden_type_fixed_param[l], hidden_value_or_meanfield="meanfield")
                            c_hat[l][k,:] = c_hat[l][k,:] + numpy.diag( HMr[l][k].T @ self.Wr[l][k] @ Xg )
                        # sample Z
                        ZMr[l],_=self.sample_h_given_b_hat(b_hat=c_hat[l], hidden_type="Bernoulli", hidden_type_fixed_param=0, hidden_value_or_meanfield="meanfield")
                        # b_hat
                        b_hat[l]=copy.deepcopy(self.br[l])
                        for k in range(self.K[l]):
                            if self.hidden_type[l]=="Bernoulli":
                                b_hat[l][k]= b_hat[l][k] + ZMr[l][k,:] * (self.Wr[l][k] @ Xg)
                            elif self.hidden_type[l]=="Gaussian":
                                b_hat[l][k][0] = b_hat[l][k][0] + ZMr[l][k,:] * (self.Wr[l][k] @ Xg)
                            # sample H
                            HMr[l][k],_ = self.sample_h_given_b_hat(b_hat=b_hat[l][k], hidden_type=self.hidden_type[l], hidden_type_fixed_param=self.hidden_type_fixed_param[l], hidden_value_or_meanfield="meanfield")
                    ...
                    for n in range(NMF):
                        # reset c_hat
                        if self.xz_interaction:
                            c_hat[l] = self.cr[l] +  self.Wr[l][self.K[l-1]] @ Zg[l-1]
                        else:
                            c_hat[l] = numpy.tile(self.cr[l], NS)
                        for k in range(self.K[l]):
                            if n==0 and not warm_start: # # initialize HMr[l]
                                HMr[l][k],_ = self.sample_h_given_b_hat(b_hat=self.repeat_bias(b_hat[l][k],NS,self.hidden_type[l]), hidden_type=self.hidden_type[l], hidden_type_fixed_param=self.hidden_type_fixed_param[l], hidden_value_or_meanfield="meanfield")
                            for k1 in range(self.K[l-1]):
                                c_hat[l][k,:]= c_hat[l][k,:] + Zg[l-1][k1,:]*numpy.diag( HMr[l][k].T @ self.Wr[l][k1][k] @ Hg[l-1][k1])
                        # sample Z
                        ZMr[l],_=self.sample_h_given_b_hat(b_hat=c_hat[l], hidden_type="Bernoulli", hidden_type_fixed_param=0, hidden_value_or_meanfield=value_or_meanfield)
                        ...
                        for k in range(self.K[l]):
                            if self.hidden_type[l]=="Bernoulli":
                                for k1 in range(self.K[l-1]):
                                    b_hat[l][k]=b_hat[l][k] + ZMr[l][k,:]*Zg[l-1][k1,:] * (self.Wr[l][k1][k] @ Hg[l-1][k1])
                            elif self.hidden_type[l]=="Gaussian":
                                for k1 in range(self.K[l-1]):
                                    b_hat[l][k][0] = b_hat[l][k][0] + ZMr[l][k,:]*Zg[l-1][k1,:] * (self.Wr[l][k1][k] @ Hg[l-1][k1])
                            # sample H
                            HMr[l][k],_ = self.sample_h_given_b_hat(b_hat=b_hat[l][k], hidden_type=self.hidden_type[l], hidden_type_fixed_param=self.hidden_type_fixed_param[l], hidden_value_or_meanfield=value_or_meanfield)
        
        return Xg,XMg,Hg,Zg,HMr,ZMr


    def generate_x(self, NS=100, NMF=20, num_iter=1000, method="Gibbs", init=True):
        """
        Generate samples from the learned exp-HM using Gibbs sampling or ancestral sampling.
        method: either "Gibbs" or "ancestral".
        """
        if method=="ancestral":
            self.Xg,self.XMg,self.Hg,self.Zg,_,_=self.sample_xhz_sleep(NS=NS, Hg=None, Zg=None, warm_start=False, NMF=NMF, value_or_meanfield="meanfield", compute_HMr=False)
            ...
            for i in range(num_iter):
                _,_,_,self.Hr,self.Zr,_,_,_=self.sample_xhz_wake(self.Xg, NMF=NMF, value_or_meanfield="meanfield", compute_HMg=False)
                self.Xg,self.XMg,self.Hg,self.Zg,_,_=self.sample_xhz_sleep(Hg=self.Hr, Zg=self.Zr, warm_start=False, NMF=NMF, value_or_meanfield="meanfield", compute_HMr=False)
        
        return self.Xg,self.XMg,self.Hg,self.Zg


    def generate_samples(self, NS=100, NMF=100, method="Gibbs", sampling_time=4, sampling_num_iter=100, row=28, col=28, dir_save="./", prefix="CHM"):
        ...
        for s in range(sampling_time):
            if s==0:
                ...
                init=False
            chainX,chainXM,chainHM,chainZM=self.generate_x(NS=NS, num_iter=sampling_num_iter, NMF=NMF, method=method, init=init)
            # plot sampled data
    ...
    def infer_hidden_states_given_x(self, train_set_x_sub=None, NMF=100, dir_save="./", prefix="CHM"):
        ...
        _,_,_,HM,ZM,_,_,_=self.sample_xhz_wake(train_set_x_sub, NMF=NMF, value_or_meanfield="meanfield", compute_HMg=False)
    ...
    def compute_gradient_sleep(self, Xfantacy, Hfantacy, Zfantacy,  HfantacyMr, ZfantacyMr):
        """
        ...
        for l in range(self.L):
            if self.hidden_type[l] in ["Bernoulli", "Poisson", "Binomial", "Multinomial", "Gaussian_FixPrecision2"]:
                ...
                for k in range(self.K[l]):
                    grad_br_lk1=-numpy.mean(Hfantacy[l][k] - HfantacyMr[l][k], axis=1)
                    grad_br_lk1.shape=(self.J[l][k],1)
                    HfantacyMr_lk2=HfantacyMr[l][k]**2 - 1/(2*self.b[l][k][1])
                    grad_br_lk2=-numpy.mean(Hfantacy[l][k]**2 - HfantacyMr_lk2, axis=1) 
                    grad_br_lk2.shape=(self.J[l][k],1)
                    self.grad_br[l][k]=[grad_br_lk1,grad_br_lk2]
        ...
        # Wr
        self.grad_Wr=copy.deepcopy(self.Wr) # just want the shape of W
        for l in range(self.L):
            if l==0:
                for k in range(self.K[l]):
                    grad_Wrlk = - ( (Zfantacy[l][k,:]*Hfantacy[l][k] - ZfantacyMr[l][k,:]*HfantacyMr[l][k]) @ Xfantacy.T )/self.batch_size
                    self.grad_Wr[l][k]=grad_Wrlk
                # W for X Omega_0 Z_0, W[0][K[0]] = Omega_0 
                if self.xz_interaction:
                    self.grad_Wr[l][self.K[l]]= - ( (Zfantacy[l] - ZfantacyMr[l]) @ Xfantacy.T )/self.batch_size
            else:# not first layer
                for k1 in range(self.K[l-1]):
                    for k in range(self.K[l]):
                        grad_Wrlk1k= - ( (Zfantacy[l][k,:]*Hfantacy[l][k] - ZfantacyMr[l][k,:]*HfantacyMr[l][k]) @ (Zfantacy[l-1][k1,:]*Hfantacy[l-1][k1]).T )/self.batch_size
                        self.grad_Wr[l][k1][k]=grad_Wrlk1k                                  
                # W for Z_l-1 Omega_l Z_l, W[l][K[l-1]] is Omega_l
                if self.xz_interaction:
                    self.grad_Wr[l][self.K[l-1]]=- ( (Zfantacy[l] - ZfantacyMr[l]) @ Zfantacy[l-1].T )/self.batch_size
    ...
    def update_param_sleep(self):
        ...
        for l in range(self.L):
            if self.hidden_type[l] in ["Bernoulli", "Binomial", "Multinomial", "Gaussian_FixPrecision1", "Gaussian_FixPrecision2"]:
                ...
                for k in range(self.K[l]):
                    self.br[l][k][0]= self.br[l][k][0] - self.learn_rate_b[l][0] * self.grad_br[l][k][0]
                    self.br[l][k][1]= self.br[l][k][1] - self.learn_rate_b[l][1] * self.grad_br[l][k][1]
                    self.br[l][k][1][self.br[l][k][1]>=0]=-tol
            ...
            if l==0:
                for k in range(self.K[l]):
                    if self.visible_type=="Multinoulli":
                        ...
                        self.Wr[l][k][0] = self.Wr[l][k][0] - self.learn_rate_W[l][0] * self.grad_Wr[l][k][0]
                        self.Wr[l][k][1] = self.Wr[l][k][1] - self.learn_rate_W[l][1] * self.grad_Wr[l][k][1]
                    else:
                ...
                for k1 in range(self.K[l-1]):
                    for k in range(self.K[l]):
                        if self.hidden_type[l]=="Gaussian2":
                            self.Wr[l][k1][k][0] = self.Wr[l][k1][k][0] - self.learn_rate_W[l][0] * self.grad_Wr[l][k1][k][0]
                            self.Wr[l][k1][k][1] = self.Wr[l][k1][k][1] - self.learn_rate_W[l][1] * self.grad_Wr[l][k1][k][1]
                        else:
    ...
    def compute_reconstruction_error(self, X0, X0RM=None):
        ...
        """
        if X0RM is None:
            X0RM,_,_,HMr,ZMr,a_hat_gen,b_hat_gen,c_hat_gen=self.sample_xhz_wake(X0, NMF=20, value_or_meanfield="meanfield", compute_HMg=False) # HMr,a_hat_gen,b_hat_gen may be used to compute the free energy as well
        else:
            HMr=None
            ZMr=None
            a_hat_gen=None
        ...
        return self.rec_error,HMr,ZMr,a_hat_gen,b_hat_gen,c_hat_gen
